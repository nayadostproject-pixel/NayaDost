...
app.use('/api',(req,res,next)=>{
 if(req.method==='GET' || !BOT_TOKEN || req.path.startsWith('/bot/'))return next();
 try{
...
// Telegram bot backend endpoints. These use a separate shared secret because
// Telegram Bot API requests do not carry Mini App initData.
function botAuth(req,res,next){
 if(!BOT_API_KEY)return res.status(503).json({ok:false,error:'BOT_API_KEY is not configured'});
 const supplied=String(req.get('X-Bot-Api-Key')||'');
 if(Buffer.byteLength(supplied)!==Buffer.byteLength(BOT_API_KEY) || !crypto.timingSafeEqual(Buffer.from(supplied),Buffer.from(BOT_API_KEY)))return res.status(401).json({ok:false,error:'Unauthorized'});
 next();
}
app.use('/api/bot',botAuth);
app.post('/api/bot/user',async(req,res)=>{try{const u=await getUser(req.body);res.json({ok:true,user:publicUser(u)})}catch(e){res.status(400).json({ok:false,error:e.message})}});
app.post('/api/bot/referral',async(req,res)=>{try{const u=await getUser(req.body);res.json({ok:true,user:publicUser(u)})}catch(e){res.status(400).json({ok:false,error:e.message})}});
