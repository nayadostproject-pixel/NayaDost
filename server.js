...
db.serialize(()=>{
 ...
 db.run(`CREATE TABLE IF NOT EXISTS telegram_task_claims(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, task_id TEXT NOT NULL, claimed_at TEXT DEFAULT CURRENT_TIMESTAMP)`);
 db.run(`CREATE INDEX IF NOT EXISTS idx_telegram_task_claims_user_task ON telegram_task_claims(user_id,task_id,claimed_at)`);
...
async function telegramStatusForUser(userId, taskId){
 const row=await get(`SELECT claimed_at FROM telegram_task_claims WHERE user_id=? AND task_id=? ORDER BY id DESC LIMIT 1`,[userId,taskId]);
...
app.post('/api/tasks/telegram-verify',async(req,res)=>{try{
 ...
 await run('BEGIN IMMEDIATE');
 try{
   const latest=await get(`SELECT claimed_at FROM telegram_task_claims WHERE user_id=? AND task_id=? ORDER BY id DESC LIMIT 1`,[u.id,task.id]);
   if(latest){
   ...
   const claimRow=await run('INSERT INTO telegram_task_claims(user_id,task_id,claimed_at) VALUES(?,?,CURRENT_TIMESTAMP)',[u.id,task.id]);
...
app.post('/api/vip/create',async(req,res)=>{try{
  ...
  const r=await run('INSERT INTO vip_payments(invoice,user_id,asset,amount_units,recipient,sender) VALUES(?,?,?,?,?,?)',[invoice,fresh.id,'USDT',units,VIP_QR_ADDRESS,(fresh.wallet_address&&fresh.verified)?fresh.wallet_address:null]);
